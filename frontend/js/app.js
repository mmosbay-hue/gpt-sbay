...
Luôn khuyến khích tự suy nghĩ trước khi đưa đáp án.`,
    ...
    // ===== Codex button =====
    const codexNavBtn = document.querySelectorAll('.sidebar-nav .sidebar-btn')[2]; // 3rd = Codex
    if (codexNavBtn) codexNavBtn.addEventListener('click', () => { window.location.href = '/gpt-builder'; });
    ...
    // ===== Try Codex for free =====
    const codexFooterBtn = document.getElementById('codexBtn');
    if (codexFooterBtn) codexFooterBtn.addEventListener('click', () => { window.location.href = '/gpt-builder'; });
