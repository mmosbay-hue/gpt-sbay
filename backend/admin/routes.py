...
def admin_questions(limit: int = 50, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    rows = conn.execute("SELECT * FROM question_logs ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
    conn.close()
    return {"questions": [dict(r) for r in rows]}


@router.get("/otp-logs")
def admin_otp_logs(limit: int = 50, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    """OTP logs."""
    import sqlite3, os
    db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "gptweb.db")
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    try:
        rows = conn.execute("SELECT * FROM otp_logs ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        conn.close()
